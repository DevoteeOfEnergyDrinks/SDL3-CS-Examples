...
internal class Program
{
    ...
    static FRect rectLeftButton = new() { X = 100, Y = 170, W = 100, H = 100 };
    static FRect rectRightButton = new() { X = 440, Y = 170, W = 100, H = 100 };
    ...
    #region Methods
    static void RenderButton(FRect rect, string str, int buttonValue)
    {
        float x, y;

        if (playingSound == buttonValue)
        {
            SetRenderDrawColor(renderer, 0, 255, 0, 255);  // green while playing
        }
        else
        {
            SetRenderDrawColor(renderer, 0, 0, 255, 255);  // blue while not playing
        }

        RenderFillRect(renderer, rect);
        SetRenderDrawColor(renderer, 255, 255, 255, 255);

        x = rect.X + ((rect.W - (DebugTextFontCharacterSize * str.Length)) / 2.0f);
        y = rect.Y + ((rect.H - DebugTextFontCharacterSize) / 2.0f);
    ...
    {
        AudioSpec spec = new() { Format = AudioFormat.AudioU8, Channels = 2, Freq = 4000 };  // Uint8 data, stereo, 4000Hz.
        ...
        if (!CreateWindowAndRenderer("examples/audio/planar-data", 640, 480, WindowFlags.Resizable, out window, out renderer))
        {
        ...
        {
            Log($"Couldn't open audio device stream: {GetError()}");
            return AppResult.Failure;
        }

        ResumeAudioStreamDevice(stream);  // SDL_OpenAudioDeviceStream starts the device paused. Resume it!
    ...
    static AppResult AppEvent(nint appstate, ref Event evt)
    {
        ...
        else if (evt.Type == (uint)EventType.MouseButtonDown)
        {
            ...
            {   // nothing currently playing?
                FPoint point = new() { X = evt.Button.X, Y = evt.Button.Y };
                if (PointInRectFloat(in point, in rectLeftButton))
                {   // clicked left button?
                    GCHandle leftHandle = GCHandle.Alloc(left, GCHandleType.Pinned);
                    IntPtr[] planes = [leftHandle.AddrOfPinnedObject(), IntPtr.Zero]; // specify NULL to say "this specific channel is silent"
                    PutAudioStreamPlanarData(stream, planes, -1, left.Length);
                    FlushAudioStream(stream);  // that's all we're playing until it completes.
                    playingSound = -1;  // left is playing
                    leftHandle.Free();
                }
                else if (PointInRectFloat(in point, in rectRightButton))
                {   // clicked right button?
                    GCHandle rightHandle = GCHandle.Alloc(right, GCHandleType.Pinned);
                    IntPtr[] planes = [IntPtr.Zero, rightHandle.AddrOfPinnedObject()]; // specify NULL to say "this specific channel is silent"
                    PutAudioStreamPlanarData(stream, planes, -1, right.Length);
                    FlushAudioStream(stream);  // that's all we're playing until it completes.
                    playingSound = 1;  // right is playing
                    rightHandle.Free();
                }
            }
        }
    ...
    {
        Delay(6);
        if (playingSound != IntPtr.Zero)
        {
            if (GetAudioStreamQueued(stream) == 0)
            {   // sound is done? We can play a new sound now.
                playingSound = 0;
            }
        }
        ...
        RenderButton(rectLeftButton, "LEFT", -1);
        RenderButton(rectRightButton, "RIGHT", 1);
