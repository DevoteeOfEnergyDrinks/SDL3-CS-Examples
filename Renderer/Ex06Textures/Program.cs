...
internal class Program
{
    ...
    {
        // The original example uses a Surface* so we use an IntPtr
        IntPtr surfacePtr;
        string pngPath;
        ...
        // engines refer to these as "sprites."
        // We'll do a static texture (upload once, draw many times) with data from a png file.
        ...
        // To get the internal values of a pointer we us PtrToStructure to get access to Width/Height
    ...
    static AppResult AppIterate(nint appstate)
    {
        ...
        // center this one.
        dstRect.X = (WindowWidth - textureWidth) / 2.0f;
        dstRect.Y = (WindowHeight - textureHeight) / 2.0f;
        dstRect.W = textureWidth;
        dstRect.H = textureHeight;
        RenderTexture(renderer, texture, IntPtr.Zero, in dstRect);

        // bottom right.
        dstRect.X = WindowWidth - textureWidth - (100.0f * scale);
        dstRect.Y = WindowHeight - textureHeight;
        dstRect.W = textureWidth;
        dstRect.H = textureHeight;
        RenderTexture(renderer, texture, IntPtr.Zero, in dstRect);
